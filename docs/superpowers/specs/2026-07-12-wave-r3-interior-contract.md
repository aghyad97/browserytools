...
- RB4 adjudications: WYSIWYG export previews (InvoiceGenerator's white PDF mockup) are CONTENT — gray/white literals allowlisted there, same class as PeriodicTable element colors. OutputPanel gains downloadSuccessMessage (silent downloads lost tool toasts). ModePicker ResizeObserver must observe segments (RTL fresh-load stale indicator).
