...
export default function YamlJsonConverter() {
  ...
  const handleConvert = () => {
    ...
    if (err) toast.error("Conversion failed");
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-7xl mx-auto space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <Button onClick={handleConvert}><ArrowRightLeft className="h-4 w-4 mr-2" />{modeLabel}</Button>
            <Button variant="outline" onClick={handleSwap}><ArrowRightLeft className="h-4 w-4 mr-2" />Swap</Button>
            <Button variant="outline" onClick={handleCopy}><Copy className="h-4 w-4 mr-2" />Copy Output</Button>
            ...
            <div className="ml-auto flex gap-2">
              <Button variant={mode === "yaml-to-json" ? "default" : "outline"} onClick={() => setMode("yaml-to-json")}>YAML→JSON</Button>
              <Button variant={mode === "json-to-yaml" ? "default" : "outline"} onClick={() => setMode("json-to-yaml")}>JSON→YAML</Button>
            </div>
          </div>
          {error && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="p-4 flex flex-col gap-2">
              ...
              <Textarea placeholder="Paste content here..." className="min-h-[420px] font-mono text-sm resize-none" value={input} onChange={(e) => setInput(e.target.value)} />
            </Card>
            <Card className="p-4 flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">{outputLabel}</span>
                {!error && output && <span className="text-xs text-green-600 dark:text-green-400">Valid</span>}
              </div>
