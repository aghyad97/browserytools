      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-7xl mx-auto space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <Button variant={direction === "text-to-binary" ? "default" : "outline"} onClick={() => setDirection("text-to-binary")}>Text → Binary</Button>
            <Button variant={direction === "binary-to-text" ? "default" : "outline"} onClick={() => setDirection("binary-to-text")}>Binary → Text</Button>
            ...
            {byteCount > 0 && direction === "text-to-binary" && (
              <span className="ml-auto text-xs text-muted-foreground">{input.length} chars / {byteCount} bytes</span>
            )}
          </div>
            ...
            <TabsContent value={tab} className="mt-4">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <Card className="p-4 flex flex-col gap-2">
                  <span className="text-sm font-medium">{direction === "text-to-binary" ? "Text Input" : tab.charAt(0).toUpperCase()+tab.slice(1)+" Input"}</span>
                  {direction === "text-to-binary" ? (
                    ...
                    <Textarea placeholder="Enter encoded value here..." className="min-h-[420px] font-mono text-sm resize-none" value={reverseInput} onChange={(e) => setReverseInput(e.target.value)} />
                  )}
                </Card>
                <Card className="p-4 flex flex-col gap-2">
                  <span className="text-sm font-medium">{direction === "text-to-binary" ? tab.charAt(0).toUpperCase()+tab.slice(1)+" Output" : "Text Output"}</span>
